...
A test runner is a small program that consumes one `.tests.json` fixture from the registry, drives a clear-signing implementation against each case, and writes a `results.json` describing what the implementation rendered. The CI workflow ([`descriptor-tests.yml`](../workflows/descriptor-tests.yml)) wraps the runner in a composite action, uploads the `results.json` as an artifact, and the results workflow ([`descriptor-test-results.yml`](../workflows/descriptor-test-results.yml)) aggregates per-implementation columns onto the PR.
...
Wrap the runner in a composite action under `.github/actions/run-<name>-tests/` and add a sibling job in [`descriptor-tests.yml`](../workflows/descriptor-tests.yml). The action should call [`upload-test-results`](../actions/upload-test-results/action.yml) to publish the artifact.
